export default {
  // First-pass translations — verify with the city translation process (Swahili especially).
  tags: {
    openNow: 'Imefunguliwa sasa',
    closed: 'Imefungwa',
    checkHours: 'Angalia saa za kazi',
    weekendHours: 'Saa za kazi za wikendi',
    openAfter6: 'Imefunguliwa baada ya saa 6 jioni',
    eveningHours: 'Saa za kazi za jioni',
    walkIns: 'Bila miadi',
    telehealth: 'Huduma ya afya kwa njia ya mtandao',
    parking: 'Maegesho ya gari hapa',
    pharmacy: 'Duka la dawa hapa',
    adultCare: 'Huduma kwa watu wazima',
    pediatrics: 'Huduma za watoto',
    multipleLanguages: 'Lugha mbalimbali',
    todayClosed: 'Leo: Imefungwa',
    todayHours: 'Leo: {range}',
  },
  providerWebsite: 'Tovuti',
  website: 'Tovuti',
  hours: 'Masaa',
  languagesSpoken: 'Lugha zinazozungumzwa na wafanyakazi',
  daysOfTheWeek: 'Siku ya wiki',
  Monday: 'Jumatatu',
  Tuesday: 'Jumanne',
  Wednesday: 'Jumatano',
  Thursday: 'Alhamisi',
  Friday: 'Ijumaa',
  Saturday: 'Jumamosi',
  Sunday: 'Jumapili',
  locationDetails: 'Maelezo ya eneo',
  contact: 'Wasiliana',
  location: 'Eneo',
  transitOptions: 'Njia za usafiri',
  servicesAvailable: 'Huduma zinazopatikana',
  patientsServed: 'Wagonjwa wanaohudumiwa',
  callout: {
    title: 'Huduma ya matibabu ya bure na ya bei ya chini huko Philadelphia',
    message:
      'Mtambuzi wetu wa huduma ya msingi unaweza kukusaidia kupata mtoa huduma huko Philadelphia. Vituo hivi vya afya vinahudumia kila mtu. Hali yako ya uhamiaji au uwezo wako wa kulipa hautakuzuia kupata huduma unayohitaji.',
    linkText: 'Pata maelezo zaidi kuhusu kupata huduma',
  },
  app: {
    aboutTitle: 'Kuhusu chombo hiki',
    title: 'Mtambuzi wa huduma ya msingi',
    name: 'Mtambuzi wa huduma ya msingi',
    subtitle: 'Huduma ya matibabu ya bure na ya bei ya chini huko Philadelphia',
    noResults:
      'Hakuna vituo vya afya vilivyopatikana. Tafadhali angalia kosa la herufi au uondoe baadhi ya vichujio vyako, na ujaribu tena.',
  },
  searchPlaceholder: 'Tafuta kwa anwani au neno kuu...',
  filters: {
    sort: 'Panga Kwa',
    distance: 'Umbali',
    name: 'Kwa alfabeti',
    ageGroup: 'Kikundi cha umri',
    adult: 'Mtu mzima',
    children: 'Watoto',
    waitTime: 'Muda wa Kusubiri (Huduma ya Msingi)',
    weekWell: 'Chini ya Wiki 1 (ziara ya uchunguzi)',
    weekSick: 'Chini ya Wiki 1 (ziara ya wagonjwa)',
    twoMonths: 'chini ya miezi 2 (huduma zote za msingi)',
    specialty: 'Huduma maalum',
    mental: 'Afya ya kiakili',
    dental: 'Meno',
    eye: 'Utunzaji wa macho',
    podiatry: 'Matibabu ya miguu',
    tests: 'Uchunguzi na picha',
    mammo: 'Eksirei ya matiti',
    xray: 'Eksirei',
    languages: 'Lugha inayozungumzwa na wafany',
  },
  introPage: {
    section1Title: 'Tafuta huduma ya msingi isiyolipishwa au ya gharama nafuu',
    p0: 'Huduma ya msingi ni nyumba yako ya utunzaji wa afya. Huduma zinajumuisha kinga, afya, na matibabu ya magonjwa ya kawaida.',
    p05: 'Vituo hivi vya afya vinahudumia kila mtu. Hali yako ya uhamiaji au uwezo wako wa kulipa hautakuzuia kupata huduma unayohitaji.',
    p1: 'Ili kupata mtoa huduma ya msingi huko Philadelphia:',
    ol1: {
      li1: 'Chuja kulingana na huduma unazohitaji.',
      li2: 'Chagua eneo kwenye ramani kwa maelezo.',
      li3: 'Wasiliana na mtoa huduma kabla ya kutembelea.',
    },
    section2Title: 'Bima na gharama',
    p2: 'Vituo hivi vya afya huchukua aina nyingi za bima. Hii ni pamoja na Medicare na Medicaid.  Unaweza kupata huduma hata kama hauna bima. Ikiwa huna bima, utalipa ada ndogo kulingana na mapato yako na ukubwa wa familia.',
    section25Title: 'Unahitaji msaada wa kupata bima ya afya?',
    p25: "Programu ya BenePhilly ya Philadelphia inatoa msaada wa bure kusaidia wakazi kuomba bima ya afya. Ili kujifunza zaidi, tembel ea <a href='https://www.phila.gov/programs/benephilly/' target='_blank'>https://www.phila.gov/programs/benephilly</a>.",
    section3Title: 'Ufikiaji wa lugha',
    p3: 'Vituo vyote vya afya vinatoa huduma za ukalimani kwa njia ya simu. Baadhi yake pia hutoa tafsiri ya ana kwa ana. Tazama maelezo ya kila eneo kwa orodha ya lugha zote zinazozungumzwa na wafanyikazi. Unaweza pia kutafuta lugha hususa.',
    section4Title: 'Kuanzisha huduma',
    p4: 'Huenda ukahitaji kuanzisha huduma na mtoa huduma kabla ya kufikia baadhi ya huduma. Piga simu kupanga miadi mpya ya mgonjwa ili uwe mgonjwa katika kituo cha afya.',
    section5Title: 'Sasisho kwa kitafutaji',
    p5: 'Tunasasisha taarifa kwa kila kituo cha afya kila baada ya miezi mitatu. Ikiwa kuna kitu kibaya, unaweza kutujulisha kupitia ',
    feedbackForm: 'fomu yetu ya maoni',
  },
  cards: {
    table1Intro:
      'Huduma hizi zinaweza kuwa maalum kwa watoto au watu wazima.  Unaweza pia kuhitaji kuwa mgonjwa, au kuanzisha huduma, katika kituo cha afya kabla ya kupata huduma fulani.',
    table2Intro:
      'Huduma zingine hazijagawanywa kulingana na umri. Baadhi yake ni wazi kwa mtu yeyote, lakini huenda ukahitaji kuanzisha huduma katika kituo hiki cha afya kabla ya kufikia vingine.',
  },
  service: 'Huduma',
  slidingScale: 'Gharama ya kipimo cha kuteleza',
  slidingScaleNull: 'Kipimo cha kuteleza hakijatolewa. Piga simu kwa habari zaidi.',
  ageSpecificServices: 'Huduma za watoto na watu wazima',
  otherServices: 'Huduma zaidi',
  ageSpecificServicesEmpty:
    'Tovuti hii haitoi huduma zozote ambazo ni za watoto au watu wazima pekee',
  otherServicesEmpty: 'Hakuna huduma zingine zinazotolewa',
  ageRange: {
    category: 'Kikundi cha umri',
    adult: 'Mtu mzima',
    child: 'Mtoto',
    adults: 'Watu wazima',
    children: 'Watoto',
    servesAdults: 'Hutumikia watu wazima.',
    servesChildren: 'Hutumikia watoto.',
    servesBoth: 'Hutumikia watu wazima na watoto.',
  },
  patientType: {
    category: 'Kustahiki',
    patient_type_new: 'Wagonjwa wapya au waliopo',
    patient_type_new_subtext:
      'Baadhi ya huduma hizi zinaweza kupatikana bila miadi. Wasiliana na mtoa huduma kwa maelezo zaidi.',
    patient_type_existing: 'Wagonjwa walioanzishwa',
    patient_type_existing_only: 'Lazima uanzishe huduma kwenye eneo',
    patient_type_existing_only_subtext:
      'Huenda ukahitaji kuwa mgonjwa katika kituo cha afya kabla ya kufikia baadhi ya huduma. Hii pia inaitwa kuanzisha huduma.',
  },
  visitType: {
    category: 'Huduma ya msingi',
    well: 'Ziara ya uchunguzi',
    sick: 'Ziara ya mgonjwa',
    sports: 'Vipimo vya uwezo wa kucheza',
    prenatal: 'Utunzaji wa ujauzito',
    women: 'Afya ya wanawake',
    telehealth: 'Matibabu kupitia mtandao au simu',
    vaccine: 'Chanjo',
  },
  tests: {
    category: 'Uchunguzi na picha',
    noTests:
      'Tovuti hii haitoi majaribio yoyote au picha zilizoorodheshwa kwenye vichujio. Piga simu ili kuulizia kuhusu vipimo vingine au picha.',
    blood: 'Maabara na vipimo vya damu',
    sti: 'Kupima magonjwa ya zinaa',
    covid: 'Kupima COVID',
    mammo: 'Eksirei ya matiti',
    xray: 'Eksirei',
  },
  languages: {
    category: 'Lugha inayozungumzwa na wafany',
    english: 'Kiingereza',
    spanish: 'Kihispania',
    chinese: 'Kichina',
    french: 'Kifaransa',
    arabic: 'Kiarabu',
    hindi: 'Kihindi',
    algerian: 'Kialjeria',
    russian: 'Kirusi',
    cambodian: 'Mkambodiani',
    'cambodian (khmer)': 'Kambodia (Khmer)',
    indonesian: 'Kiindonesia',
    mandarin: 'Kimandarini',
    asl: 'ASL',
    bengali: 'Kibangali',
    bengalis: 'Kibangali',
    vietnamese: 'Kivietinamu',
    malayalam: 'Malajalamali',
    malayim: 'Malajalamali',
    fanta: 'Fanta',
    urdu: 'Urdu',
    hebrew: 'Kiebrania',
    tagalog: 'Kitagalogi',
    gujarati: 'Kigujarati',
    portuguese: 'Kireno',
    portugese: 'Kireno',
    'brazilian portuguese': 'Kireno ya Brazil',
    'french creole': 'Kreyole ya Haiti',
    'french-creole': 'Kreyole ya Haiti',
    karen: 'Karen',
    yoruba: 'Kiyoruba',
    telugu: 'Mtelugu',
    kinyarwanda: 'Kinyarwanda',
    khmer: 'Khmer',
    swahili: 'Swahili',
    kirundi: 'Kirundi',
    cantonese: 'Kikantoni',
    burmese: 'Kiburma',
    creole: 'Kreyole ya Haiti',
    korean: 'Kikorea',
    'haitian creole': 'Kreyole ya Haiti',
    'haitian-creole': 'Kreyole ya Haiti',
    haitianCreole: 'Kreyole ya Haiti',
    taiwanese: 'Kitaiwani',
    'bahasa indonesian': 'Bahasa Indonesia',
    italian: 'Kiitaliano',
    chin: 'Kifungu',
    filipino: 'Kifilipino',
    farsi: 'Kifarsi',
    ukrainian: 'Mkreni',
    german: 'Mjerumani',
    hmong: 'Hmong',
    japanese: 'Kijapani',
    lao: 'Lao',
    nepali: 'Kinepali',
    shan: 'Shan',
    thai: 'Thai',
    dari: 'Dari',
    dutch: 'Kiholanzi',
    pashtu: 'Pashtu',
    tigrinya: 'Tigrinya',
    mandingo: 'Mandingo',
    bambara: 'Bambara',
    ewe: 'Ewe',
    krio: 'Krio',
    patois: 'Patois',
    pular: 'Pular',
    'farsi and malayalam': 'Farsi na Malajalamali',
  },
  specialty: {
    category: 'Huduma maalum',
    mental: 'Afya ya kiakili',
    dental: 'Meno',
    eye: 'Utunzaji wa macho',
    podiatry: 'Matibabu ya miguu',
    mat: 'Matibabu ya matumizi ya dawa (MAT)',
    nutrition: 'Ushauri wa lishe',
    tobacco: 'Msaada wa kuacha tumbaku',
    pharmacy: 'Duka la dawa kwenye eneo',
  },
  waitTime: {
    category: 'Muda wa Kusubiri (Huduma ya Msingi)',
    walkIn: 'Siku hiyo hiyo au bila miadi (ziara ukiwa mgonjwa)',
    oneWeekWell: 'Chini ya Wiki 1 (ziara ya uchunguzi)',
    oneWeekSick: 'Chini ya Wiki 1 (ziara ya wagonjwa)',
    twoMonths: 'chini ya miezi 2 (huduma zote za msingi)',
  },
  transit: {
    bus: 'Basi',
    subway: {
      label: 'Njia ya chini ya ardhi',
      BSL: 'Broad Street Line (orange line)',
      MLF: 'Market-Frankford Line (blue line)',
    },
    regRail: {
      label: 'Reli za eneo',
      AL: 'Airport Line',
      CHE: 'Chestnut Hill East Line',
      CHW: 'Chestnut Hill West Line',
      CL: 'Cynwyd Line',
      FC: 'Fox Chase Line',
      GC: 'Glenside Combined',
      LD: 'Lansdale/Doylestown Line',
      PM: 'Center City hadi Penn Medicine Station',
      MN: 'Manayunk/Norristown Line',
      ME: 'Media/Elwyn Line',
      PT: 'Paoli/Thorndale Line',
      TL: 'Trenton Line',
      WL: 'Warminster Line',
      WTL: 'West Trenton Line',
      WN: 'Wilmington/Newark Line',
      FR: 'Fern Rock hadi Center City',
    },
    trolley: 'Toroli',
    car: {
      label: 'Gari',
      OST: 'Maegesho ya barabarani yanapatikana',
      MOST: 'Maegesho ya barabarani yenye mashine ya kulipia yanapatikana',
      GP: 'Maegesho ya gereji yanapatikana',
      FG: 'Maegesho ya gereji yasiyo ya kulipia yanapatikana',
      OS: 'Maegesho kwenye eneo yanapatikana',
      FOS: 'Maegesho ya bure kwenye eneo yanapatikana',
      RV: 'Inahitaji kuthibitishwa',
      PL: 'Nafasi ya maegesho ya barabarani inapatikana',
    },
  },
  tooltips: {
    well: 'Inajumuisha vipimo vya kimwili',
    mat: 'Dawa za ugonjwa wa matumizi ya opioid',
    women: 'Inajumuisha upangaji uzazi na utunzaji wa uzazi',
  },
  caveats: {
    'Exclusively serves adolescents and young adults (ages 13-26).':
      'Hutumikia pekee vijana balehe na vijana wazima (wenye umri wa miaka 13-26).',
    'Exclusively serves people living with HIV.': 'Hutumikia pekee watu wanaoishi na VVU.',
    "Exclusively serves uninsured members of Philadelphia's Latine immigrant community.  No work physicals.  Services for children are limited to eye care and mental health.":
      'Hutumikia pekee wanajamii wahamiaji wa Latine wasio na bima ya afya katika Philadelphia. Hakuna uchunguzi wa afya kwa ajili ya kazi. Huduma kwa watoto zimepunguzwa kwa macho na afya ya akili.',
    'Services for children are limited to dental, mental health, and vaccines.':
      'Huduma kwa watoto zimepunguzwa kwa meno, afya ya akili, na chanjo.',
    'Special focus on LGBTQ+ ': 'Inaangazia hasa LGBTQ+',
    'Special focus on people who are currently or recently homeless, or at risk of homelessness.':
      'Inaangazia hasa watu ambao kwa sasa au hivi karibuni hawana makazi, au walio katika hatari ya kukosa makazi.',
  },
  warnings: {
    'This health center only offers HIV and STD/STI testing and treatment; walk-ins only ':
      'Kituo hiki cha afya hutoa tu upimaji na matibabu ya VVU na STD/STI; kuendelea tu',
    'No in-person visits available.': 'Hakuna ziara za kibinafsi zinazopatikana.',
    'This health center only sees people who are living with HIV.':
      'Kituo hiki cha afya kinawaona watu wanaoishi na VVU pekee.',
    'COVID-19 vaccines and testing are available to everyone. For all other services, this health center only sees people who are immigrants, undocumented, or who speak Spanish.':
      'Chanjo na majaribio ya COVID-19 yanapatikana kwa kila mtu. Kwa huduma zingine zote, kituo hiki cha afya huona tu watu ambao ni wahamiaji, wasio na hati au wanaozungumza Kihispania.',
    'Telehealth appointments for established patients only. New patient telehealth appointments available only for patients starting Hep C treatment.':
      'Uteuzi wa Telehealth kwa wagonjwa waliowekwa tu. Uteuzi mpya wa telehealth ya mgonjwa unapatikana tu kwa wagonjwa wanaoanza matibabu ya Hep C.',
    'New patients should call on the first Wednesday of the month to make an appointment.':
      'Wagonjwa wapya wanapaswa kupiga simu Jumatano ya kwanza ya mwezi kufanya miadi.',
    'The pharmacy is currently closed for construction':
      'Duka la dawa kwa sasa imefungwa kwa ujenzi.',
    'Services only offered for patients ages 14+':
      'Huduma zinazotolewa tu kwa wagonjwa wenye umri wa miaka',
  },
  exceptions: {
    'Open first and third Saturday of each month':
      'Hufunguliwa Jumamosi ya kwanza na ya tatu ya mwezi',
    'Patients seen by appointment only from 4:30 p.m. - 8 p.m.':
      'Wagonjwa huonwa kwa miadi tu kutoka saa 10:30 jioni (4:30 p.m.) - Saa 12:30 jioni (8 p.m.)',
    'Telehealth visits only': 'Ziara kupitia mtandao au simu pekee',
    'On first Wednesday of the month, clinic opens at 1 p.m.':
      'Jumatano ya kwanza ya mwezi, kliniki inafunguliwa saa 1 jioni.',
    'Open until 8 p.m. by appointment only ': 'Hufunguliwa hadi saa 2 usiku (8 p.m.) kwa miadi tu',
    'Closed noon - 1 p.m.': 'Hufungwa saa 6 (12 p.m.) – saa 7 mchana (1 p.m.)',
    'Closed at noon for lunch': 'Ilifungwa saa sita mchana kwa chakula cha mchana',
    'Urgent care available from 10:30 a.m. - 2 p.m.':
      'Huduma ya haraka inapatikana kutoka 10:30 a.m. - saa 8 mchana (2 p.m.)',
    'On second Friday of the month, clinic closes at 1 p.m.':
      'Siku ya Ijumaa ya pili ya mwezi, kliniki inafungwa saa 1 jioni',
    'On second Saturday of the month, clinic is open from 10 a.m. to 2 p.m. ':
      'Jumamosi ya pili ya mwezi, kliniki inafunguliwa kutoka 10 asubuhi - 2 jioni',
    'Patients seen by appointment only until 7:30 p.m. on the first and third Thursdays of the month':
      'Wagonjwa wanaonekana kwa miadi tu hadi 7:30 jioni siku ya Alhamisi ya kwanza na ya tatu ya mwezi',
    'Open first and third Thursday of each month from 5 p.m. - 7:30 p.m. (appointment only)':
      'Fungua Alhamisi ya kwanza na ya tatu ya kila mwezi kutoka 5 jioni - 7:30 jioni (uteuzi tu)',
    'Open on second Saturday of the month': 'Fungua Jumamosi ya pili ya mwezi',
    'Telehealth available 5 p.m. - 7 p.m.': 'Telehealth inapatikana 5 jioni - 7 jioni.',
    'Closes at noon on the third Thursday of every month, Telehealth available 5 p.m. - 7 p.m.':
      'Inafungwa saa mchana Alhamisi ya tatu ya kila mwezi, Telehealth inapatikana 5 jioni - 7 jioni.',
    'Dental available 8:30 a.m. - 1 p.m. every third Saturday':
      'Meno inapatikana 8:30 asubuhi - 1 jioni kila Jumamosi ya tatu',
    'Open every fourth Saturday of the month for primary care and podiatry':
      'Fungua kila Jumamosi ya nne ya mwezi kwa huduma ya msingi na podiatry',
    'Open 8:30 a.m. - 1 p.m. every second and fourth Saturday of the month':
      'Fungua 8:30 asubuhi - 1 jioni kila Jumamosi ya pili na ya nne ya mwezi',
    'Dental and pediatric services available 8:30 a.m. - 1 p.m. every second and fourth Saturday of the month':
      'Huduma za meno na watoto zinapatikana 8:30 asubuhi - 1 jioni kila Jumamosi ya pili na nne ya mwezi',
    'Open 8:30 a.m. - 12 p.m. every third Friday of the month':
      'Fungua 8:30 asubuhi - 12 jioni kila Ijumaa ya tatu ya mwezi',
    'Open 8:30 a.m. - 12 p.m. every third Saturday of the month':
      'Fungua 8:30 asubuhi - 12 jioni kila Jumamosi ya tatu ya mwezi',
    'On fourth Friday of the month, clinic opens from 10 a.m. - 5 p.m.':
      'Siku ya Ijumaa ya nne ya mwezi, kliniki inafungua kutoka 10 asubuhi hadi 5 jioni.',
    'On fourth Friday of the month, clinic opens 10 a.m. - 5 p.m.':
      'Siku ya Ijumaa ya nne ya mwezi, kliniki inafungua kutoka 10 asubuhi hadi 5 jioni.',
    'Closed 4 p.m. - 5 p.m.': 'Imefungwa 4 jioni - 5 jioni.',
    'Closes at 1 p.m. every second Friday of the month':
      'Inafungwa saa 1 jioni kila Ijumaa ya pili ya mwezi',
    'Closes at noon on the third Thursday of every month':
      'Inafungwa mchana siku ya Alhamisi ya tatu ya kila mwezi',
    'On 2nd and 4th Tuesday of the month, clinic is open from 7:30 a.m. - 8:00 p.m. ':
      'Jumanne ya 2 na 4 ya mwezi, kliniki iko wazi kutoka 7:30 asubuhi - 8:00 jioni.',
    'On 2nd Thursday of the month, clinic is open from 9:00 a.m. - 5:00 p.m.':
      'Siku ya Alhamisi ya 2 ya mwezi kliniki iko wazi kutoka 9:00 asubuhi hadi 5:00 jioni.',
    'On 2nd Friday of the month, clinic is open from 1:30 p.m. - 5:00 p.m.':
      'Siku ya Ijumaa ya 2 ya mwezi kliniki iko wazi kutoka 1:30 jioni - 5:00 jioni.',
    'On second and fourth Saturday of the month, clinic is open from 8:30 a.m. - 1 p.m. for dental services':
      'Jumamosi ya pili na ya nne ya mwezi, kliniki iko wazi kutoka 8:30 asubuhi - 1 jioni kwa huduma za meno',
    'Evening telehealth services 5 p.m. - 8 p.m.': 'Huduma za telehealth jioni 5 jioni - 8 jioni.',
    'Evening telehealth services available 5 p.m. - 8 p.m.':
      'Huduma za telehealth jioni 5 jioni - 8 jioni.',
    'Evening Telehealth service 5 p.m. to 7 p.m.': 'Huduma ya Telehealth jioni 5 jioni - 7 jioni.',
    'Evening telehealth services 5 p.m. - 8 p.m. Open every fourth Saturday of the month for primary care and podiatry':
      'Huduma za telehealth jioni 5 jioni - 8 jioni Inafunguliwa kila Jumamosi ya nne ya mwezi kwa huduma ya msingi na podiatry',
    'On third Thursday of the month, clinic is closed':
      'Siku ya Alhamisi ya tatu ya mwezi, kliniki imefungwa',
    'On second Friday of the month, clinic closes at noon':
      'Ijumaa ya pili ya mwezi, kliniki inafungwa mchana',
    'Open 5 p.m. - 8 p.m. by appointment only': 'Fungua saa 5 jioni hadi 8 jioni kwa miadi tu',
    'Select Saturdays by appointment only': 'Kwa uteuzi tu siku ya Jumamosi kadhaa',
    'Open until 7 p.m. by appointment only ': 'Fungua hadi saa 7 jioni kwa miadi tu',
    'On first and third Thursday of the month, clinic is open until 7:30 p.m.':
      'Siku ya Alhamisi ya kwanza na ya tatu ya mwezi, kliniki iko wazi hadi 7:30 jioni.',
    'Evening telehealth services available 5 p.m. - 6 p.m.':
      'Huduma za telehealth za jioni zinapatikana 5 jioni - 6 jioni.',
    'Dental services available 8:30 a.m. - 1 p.m.':
      'Huduma za meno zinapatikana 8:30 asubuhi - 1 jioni.',
    'Evening telehealth services available 5 p.m. - 7 p.m.':
      'Huduma za telehealth za jioni zinapatikana 5 jioni - 7 jioni.',
    'Evening telehealth services 5 p.m. to 7 p.m.':
      'Huduma za telehealth za jioni zinapatikana 5 jioni - 7 jioni.',
    'Open every first and third Saturday of the month':
      'Fungua kila Jumamosi ya kwanza na ya tatu ya mwezi',
    'Medical services are provided Thursday evenings only':
      'Huduma za matibabu hutolewa Alhamisi jioni tu',
    'On 2nd and 4th Saturday of the month, clinic is open from 7:00 a.m. - noon':
      'Jumamosi ya 2 na 4 ya mwezi, kliniki iko wazi kutoka 7 asubuhi - mchana',
    'Every other Saturday, clinic is open by appointment only':
      'Kila Jumamosi nyingine, kliniki inafunguliwa kwa miadi tu',
    'On 2nd and 4th Saturday of the month, clinic is open from 8:30 a.m. - 1 p.m. for dental services':
      'Jumamosi ya 2 na 4 ya mwezi, kliniki iko wazi kutoka 8:30 asubuhi - 1 jioni kwa huduma za meno',
    'On 1st and 3rd Saturday of the month, clinic is open for dental services':
      'Jumamosi ya 1 na 3 ya mwezi, kliniki iko wazi kwa huduma za meno',
    'On 2nd Saturday of the month, clinic is open from 10 a.m. - 2 p.m.':
      'Jumamosi ya 2 ya mwezi, kliniki iko wazi kutoka 10 asubuhi hadi 2 jioni.',
    'Telehealth visits only from 9 a.m. - 3 p.m.':
      'Ziara za telehealth tu kutoka 9 asubuhi hadi 3 jioni.',
  },
  closed: 'Imefungwa',
  'closed*': 'Imefungwa*',
  slidingScaleExplanation:
    'Hivi ndivyo unavyoweza kutarajia kulipa kwa ziara ya kawaida ya matibabu. Gharama yako itategemea mapato yako na ukubwa wa familia.',
  share: 'Shiriki',
  results: 'matokeo',
  outOf: 'nje ya',
  backToHome: 'Rudi kwenye ukurasa',
  back: 'Nyuma',
  currentLocation: 'Mahali pa sasa',
  useCurrentLocation: 'Tumia eneo la sasa',
  searchBar: 'Search bar',
  searchButton: 'Search kifungo',
  otherLanguages:
    'Ikiwa unatafuta lugha tofauti, unaweza kuiandika kwenye upau wa utafutaji kwenye ramani',
  tableNoData: {
    noSpecializedServices:
      'Kituo hiki cha afya hakitoi huduma zozote ambazo zimepunguzwa kwa watoto au watu wazima',
    noOtherServices: 'Kituo hiki cha afya hakitoi huduma zingine yoyote',
    noHours: 'Hakuna masaa inayojulikana kwa kituo hiki cha afya',
  },
}
