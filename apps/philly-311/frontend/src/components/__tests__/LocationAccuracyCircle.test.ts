...
// ABOUTME: fix's lng/lat with a recentering offset, and the circle div is sized in
// ABOUTME: pixels from the accuracy radius at that latitude/zoom.
...
describe('LocationAccuracyCircle', () => {
  it('places the MapMarker at the fix lng/lat, offset down by the pixel radius to recenter', () => {
    ...
    expect(mapMarkerProps).toHaveBeenCalledWith(
      expect.objectContaining({ lngLat: [-75.16, 39.95], offset: [0, radius] }),
    )
  })
  ...
  it('recomputes the offset and size when zoom changes', async () => {
    const w = mount(LocationAccuracyCircle, {
      props: { latitude: 39.95, longitude: -75.16, accuracy: 10, zoom: 16 },
    })
    await w.setProps({ zoom: 17 })
    const radius = accuracyRadiusPixels(10, 39.95, 17)
    expect(mapMarkerProps).toHaveBeenLastCalledWith(
      expect.objectContaining({ offset: [0, radius] }),
    )
