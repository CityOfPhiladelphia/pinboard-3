...
export function useSearchAddress(address: string | Ref<string>) {
  ...
  async function getAddressCoordinatesFromAIS() {
    ...
    const url = `${import.meta.env.DEV ? import.meta.env.VITE_AIS_URL : 'https://haydr3k097.execute-api.us-east-1.amazonaws.com/queryAis/coordinate?address='}${encodeURIComponent(addressDeref)}`
